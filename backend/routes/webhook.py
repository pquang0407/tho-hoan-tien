...
async def global_postback(request: Request):
    ...
    if request.method == "POST":
        try:
            ...
            print(f"Error parsing POST JSON body: {ex}")
    ...
    try:
        ...
        print(f"ERROR: Failed to save order to Firestore: {db_err}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Database write failure: {str(db_err)}")
